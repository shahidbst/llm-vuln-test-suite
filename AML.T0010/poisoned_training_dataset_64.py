...
# CWE: CWE-749
# SEVERITY: Critical
# DESCRIPTION: Training dataset loaded from unverified source contains poisoned samples, variant 64
...
dataset = load_dataset("community-user/training-data-64")  # may be poisoned
from transformers import Trainer, TrainingArguments, AutoModelForCausalLM
model = AutoModelForCausalLM.from_pretrained("gpt2")
trainer = Trainer(model=model, train_dataset=dataset["train"],
    args=TrainingArguments(output_dir=f"/tmp/finetuned-64", num_train_epochs=3))
