...
# CWE: CWE-200
# SEVERITY: High
# DESCRIPTION: Reconstruction quality scores exposed enabling iterative model inversion, variant 73
# EXPLOIT: Iteratively craft inputs to maximize reconstruction score, inverting private training features

import openai
from flask import Flask, request, jsonify
app = Flask(__name__)
@app.route("/reconstruct/73", methods=["POST"])
def reconstruct_73():
    candidate = request.json["candidate"]
    # Model rates how well candidate reconstructs a private target
    resp = openai.ChatCompletion.create(model="ft:gpt-3.5-turbo:org:private-73",
