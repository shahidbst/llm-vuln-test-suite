...
# CWE: CWE-200
# SEVERITY: High
# DESCRIPTION: Attribute inference enabled by detailed model outputs, variant 44
# EXPLOIT: Query model about person with partial info; infer private attributes from output distribution

import openai
from flask import Flask, request, jsonify
app = Flask(__name__)
@app.route("/infer-attr/44", methods=["POST"])
def attr_44():
    partial_profile = request.json["profile"]
    resp = openai.ChatCompletion.create(
        model="ft:gpt-3.5-turbo:org:medical-records-44",
