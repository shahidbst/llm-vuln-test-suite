...
# CWE: CWE-200
# SEVERITY: High
# DESCRIPTION: Black-box model inversion via output-only queries, variant 98
# EXPLOIT: Train local inversion model on (input, output) pairs to reverse-engineer training data

from flask import Flask, request, jsonify
import openai
app = Flask(__name__)
@app.route("/classify-detailed/98", methods=["POST"])
def classify_98():
    text = request.json["text"]
    resp = openai.ChatCompletion.create(model="ft:gpt-3.5-turbo:org:dataset-98",
