...
# CWE: CWE-200
# SEVERITY: Critical
# DESCRIPTION: Feature reconstruction attack via repeated model queries, variant 5
...
import torch
app = Flask(__name__)
model = AutoModelForCausalLM.from_pretrained("org/sensitive-5")
tok = AutoTokenizer.from_pretrained("org/sensitive-5")
@app.route("/features/5", methods=["POST"])
def feats_5():
