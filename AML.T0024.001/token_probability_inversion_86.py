...
# CWE: CWE-200
# SEVERITY: High
# DESCRIPTION: Token probabilities returned enable reconstruction of private training text, variant 86
# EXPLOIT: Use output token distribution to recover original training sequences via beam search

import openai
from flask import Flask, request, jsonify
app = Flask(__name__)
@app.route("/token-probs/86", methods=["POST"])
def tprobs_86():
    resp = openai.ChatCompletion.create(model="ft:gpt-3.5-turbo:org:private-corpus-86",
