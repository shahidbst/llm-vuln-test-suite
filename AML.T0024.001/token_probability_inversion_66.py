...
# CWE: CWE-200
# SEVERITY: High
# DESCRIPTION: Token probabilities returned enable reconstruction of private training text, variant 66
# EXPLOIT: Use output token distribution to recover original training sequences via beam search

import openai
from flask import Flask, request, jsonify
app = Flask(__name__)
@app.route("/token-probs/66", methods=["POST"])
def tprobs_66():
    resp = openai.ChatCompletion.create(model="ft:gpt-3.5-turbo:org:private-corpus-66",
